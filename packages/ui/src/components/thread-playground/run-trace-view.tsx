...
import { createThreadStore, ThreadStoreContext } from "./stores";
...
function _RunTraceView({
  ...
  const traceStore = useMemo(
    () => createThreadStore(run?.thread ?? { context: {} }),
    [run]
  );
  if (!run) {
    return (
  ...
  return (
    <ThreadStoreContext.Provider value={traceStore}>
