...
- 可见缺口：三款可见模型普通 Run、GLM-4.6V 图片问答和“模型发起天气 Tool Call—自动执行—模型整理答案”的完整天气 ReAct 闭环均已在线验证。首次成功状态现在会跨刷新保留最后一次失败/停止的脱敏摘要与恢复入口，但仍没有完整失败时间线、后台重试或静默故障转移。公共远程 MCP 上线前还需腾讯云网络层出站限制；BYOK、登录激活、租户级 Thread CRUD、多标签 Workspace、分布式额度、审计监控、备份和更强滥用防护仍待开发。
...
- 可见缺口：Web 首次成功 V1 已部署首次天气 Run 模式选择与内容无关激活事件，但仍没有达到 30 个样本的真实访客漏斗基线；其他案例仍继承全局 Run 模式。

## 游客核心迭代闭环

- 状态：Web 首次成功与通用激活漏斗 V1 已提交、推送、部署腾讯云并完成线上真实浏览器验收；运行历史与评估流程中文化仍待处理
- 新鲜度：confirmed
- 最后检查：2026-08-23
- 证据：
  - 2026-08-23 线上全新 Chromium 从 20/20 额度进入首次选择，完整模式实际完成两次 `/api/guest/runs` 与一次 `/api/guest/tools/call`，全部 200；额度变为 18/20，页面展示真实 `weather_report` 结果、最终天气答案、成功 CTA 和 3-message Run history。
  - 点击“查看运行记录”后 CTA 正确消失；刷新恢复 Thread、工具结果、最终答案和 Run history，再次点击 Run 不重复显示首次选择。该线上会话 console 为 0 error / 0 warning，Models、Quota 与 PostHog 请求均为 200。
  - 线上 1440px 模式选择与完成态、390px 模式选择均完成截图审计；390px 实测 `scrollWidth === innerWidth === 390`。错误 Origin 返回 403、私网 Web Fetch 返回 400、公共远程 MCP 返回 403。
  - 发布前全仓零警告 lint 与全仓 typecheck 通过，GitHub Linux CI 成功。Windows 完整测试为 831 pass / 1 skip / 24 fail，失败均位于未改动的 `/bin/sh`、`python3`、symlink/权限、POSIX 路径和生成文件换行基线；本轮相关 68 项为 68/68 通过。
  ...
  - 当前受控上线验收样本为 1/1，生产北极星仍未达到统计基线：7 天内至少 30 个隐私安全样本时，首次访客完整 Agent 闭环完成率目标为 `>=60%`，中位完成时间目标为 `<=5 分钟`。
...
- 可见缺口：首次天气闭环和通用激活漏斗已经部署，但生产基线仍需至少 30 个隐私安全样本；“修改—第二次运行—检查/比较—保存评估”的后半段渐进式闭环、历史入口可发现性，以及 Run history、Compare、Inspect、Evaluate 等流程中文化仍待处理。
...
- 证据：
  ...
  - 功能提交 `a4c9c7e` 与可达性修复 `5901d8b` 已推送；腾讯云当前发布 `20260813-170635-5901d8b3c62b`，服务 active，Web/API 软链接一致，Nginx 检查通过。
...
- Evidence:
  ...
  - 2026-08-23 Tencent Cloud release `20260823-123702-6b3eeb819161` deployed commit `6b3eeb819161df80a797de341f4f6770f162a017`; the active API and Nginx services remained running with no warning-level service log entries after acceptance.
  - Fresh production Chromium completed a real 1-case / 2-variant run at `https://kandian.site/llm-space-web/#/workbench`: quota moved from 20 to 18, both sequential `/api/guest/runs` requests returned 200, both variants completed in one model turn with the required-text check passing, traces exposed messages/model/usage/timing, a Tie evaluation and note survived reload, and ordinary Run history remained empty.
